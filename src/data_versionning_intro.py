...
# ## What are the tools ?
#
# In the area of code versioning, there are a few tools available but [Git](https://git-scm.com) clearly dominates the area. When it comes to data versioning, a relatively younger field, there are still a lot of available tools. We won't focus on enumerating them all here, but we will name the most popular ones and give some pointers:
#
...
# - [Git-annex](https://git-annex.branchable.com) allows managing large files with [Git](https://git-scm.com), without storing the file contents in [Git](https://git-scm.com). It is a lower-level tool than [DataLad](https://www.datalad.org) and [DVC](https://dvc.org) which expose higher level abstractions to users.
...
# - [DVC](https://dvc.org) See next sections.
...
#
# That being said, you might be better using another tool. There are some dicussions ([here](https://discuss.dvc.org/t/comparison-to-datalad-and-git-annex/92) for example) trying to compare these tools.
#
# We strongly encourage you to make your own opinion by taking a closer look at them yourself. If you are interested in a low-level data versioning tool and not too afraid by an old-looking documentation, you might be better off with [Git-annex](https://git-annex.branchable.com). If you are more interested in easily discovering datasets published by others, you might want to try [DataLad](https://www.datalad.org).
#
...
# ## What is DVC ?
#
# [DVC](https://dvc.org) is a tool for data science that takes advantage of existing software engineering toolset. It helps scientific teams manage large datasets, make projects reproducible, and collaborate better.
#
# ```{warning}
